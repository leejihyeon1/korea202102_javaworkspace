...
public class ButtonMain extends JFrame implements ActionListener{
	...
	/*배열이 나쁜건 아니지만, 선언시에 반드시 그 크기를 정해야 한다는 것은 프로그램 개발시 유연성에 있어서는 불편하다..
	 * 그렇다면 자바스크립트처럼 배열의 크기가 동적으로 늘어날 수 있는 형태의 객체는 없을까??
	JButton[] btnArray2=new JButton[];//낱개로 생성된 버튼들을 담게될 배열
	*/
	
	//배열과 거의 같지만, 크기가 동적으로 변할 수 있는 컬렉션프레임웍 중 List계열을 사용해보자!!
	...
	public void create() {
		...
		btnArray=new JButton[ea];//배열의 크기를 결정지을 수 있음 , 따라서 이때 배열 생성 가능
		
		//2-입력한 수 만큼 버튼 생성하여 p_center에 붙이기
		for(int i=0; i<ea; i++) {
